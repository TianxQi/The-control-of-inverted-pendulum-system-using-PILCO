...
class IPEnv():
   ...
   # metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 50}
    ...
    def step(self, action):
        ...
        velocity_penalty =(0.004 * (theta_dot ** 2)) + (0.01 * (x_dot ** 2))#速度惩罚项np.abs(x_dot)/2# (0.001 * (theta_dot ** 2)) + (0.005 * (x_dot ** 2))#速度惩罚项
        angel_reward2 =5*np.exp(-np.abs(theta/0.5))-0.5
        ...
        reward =0 -dist_penalty+angel_reward2-velocity_penalty#postion_reward - velocity_penalty
            ...
            self.counter > 100 or x < -self.x_threshold or x > self.x_threshold)  # or theta > 90*2*np.pi/360  or theta < -90*2*np.pi/360)
        if done:
    ...
    # Reset the state
        self.state = np.array([
           np.random.uniform(low=-0.118, high=-0.118),  # x
           np.random.uniform(low=0.015, high=0.015),  # x_dot
           np.random.uniform(low=0.0177, high=0.0177),  # theta np.pi
           np.random.uniform(low=-0.046, high=-0.046) # theta_dot
        ])
