...
import os
import pickle
def save_pilco(pilco, filename='pilco_model.pkl'):
...
#load_model_if_exists(pilco, 'best_model01.pkl') # 加载模型
best_reward = load_best_reward()
#for k in range (2):
for rollouts in range(5):
    ...
    if reward_ges > best_reward:  # 如果本次迭代的reward超过了原先的reward
        ...
        save_pilco(pilco,'pilco_model.pkl')
...
# plt.plot(REWARD)
# plt.show()
save_REWARD(REWARD, f'reward_data.pkl')
print(REWARD)
