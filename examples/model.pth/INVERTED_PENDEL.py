...
X, Y, _, _ = rollout(env=env, pilco=None, random=True, timesteps=4700, render=True)
for i in range(1, 5):  # 迭代 4 次
    X_, Y_, _, _ = rollout(env=env, pilco=None, random=True, timesteps=4700, render=True)
    X = np.vstack((X, X_))
    Y = np.vstack((Y, Y_))
X1 = X
Y1 = Y#减小内存
...
for k in range (2):
    for rollouts in range(2):
        ...
        save_REWARD(REWARD, f'reward_data_{rollouts+100*(k+1)}.pkl')
    print('The RUN times:',k+1)
    X = X1
    Y = Y1
